...
const Navigation: React.FC<NavigationProps> = ({ currentView, setView, onOpenAI }) => {
  return (
    <div className="fixed bottom-0 left-0 w-full bg-white border-t-4 border-pop-dark pb-safe pt-2 z-[1000]">
      <div className="max-w-md mx-auto px-2">
        <div className="grid grid-cols-5 h-20 items-end pb-4 gap-1">
          <button
            onClick={() => setView('itinerary')}
            className={`group flex flex-col items-center justify-center h-full w-full gap-1 rounded-xl transition-all active:translate-y-1 ${currentView === 'itinerary' ? 'text-pop-blue' : 'text-gray-400 hover:text-pop-dark'
              }`}
          >
            <div className={`p-2 rounded-lg border-2 transition-all ${currentView === 'itinerary'
          ...
          <button
            onClick={() => setView('map')}
            className={`group flex flex-col items-center justify-center h-full w-full gap-1 rounded-xl transition-all active:translate-y-1 ${currentView === 'map' ? 'text-pop-blue' : 'text-gray-400 hover:text-pop-dark'
              }`}
          >
            <div className={`p-2 rounded-lg border-2 transition-all ${currentView === 'map'
                ? 'bg-pop-yellow border-pop-dark shadow-pop-sm -translate-y-1'
                : 'bg-transparent border-transparent'
              }`}>
          ...
          <button
            ...
            className={`group flex flex-col items-center justify-center h-full w-full gap-1 rounded-xl transition-all active:translate-y-1 ${currentView === 'expenses' ? 'text-pop-blue' : 'text-gray-400 hover:text-pop-dark'
              }`}
          >
            <div className={`p-2 rounded-lg border-2 transition-all ${currentView === 'expenses'
                ? 'bg-pop-yellow border-pop-dark shadow-pop-sm -translate-y-1'
                : 'bg-transparent border-transparent'
              }`}>
          ...
          <button
            ...
            className={`group flex flex-col items-center justify-center h-full w-full gap-1 rounded-xl transition-all active:translate-y-1 ${currentView === 'wallet' ? 'text-pop-blue' : 'text-gray-400 hover:text-pop-dark'
              }`}
          >
            <div className={`p-2 rounded-lg border-2 transition-all ${currentView === 'wallet'
                ? 'bg-pop-yellow border-pop-dark shadow-pop-sm -translate-y-1'
                : 'bg-transparent border-transparent'
              }`}>
