...
import { ChatMessage, Activity, ActivityType, Currency } from '../types';
...
const AIChat: React.FC<AIChatProps> = ({ isOpen, onClose, itinerary, onUpdateItinerary }) => {
    ...
    const handleSend = async () => {
        ...
        const suggestedActivities = await generateItinerarySuggestion(itinerary, input, currentDate);
        ...
        if (suggestedActivities && suggestedActivities.length > 0) {
            ...
            setMessages(prev => [...prev, {
                id: Date.now().toString(),
                role: 'model',
                text: 'OOF. It looks like the AI is currently exhausted or reached its quota. Please check your API key status or wait a moment before trying again.'
            }]);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none">
            ...
            {/* Chat Window - True Floating Modal */}
            <div className="pointer-events-auto bg-white w-full max-w-[400px] h-full max-h-[80vh] rounded-[32px] shadow-[8px_8px_0px_0px_#18181b] flex flex-col overflow-hidden animate-slide-up relative border-4 border-pop-dark">
