...
const getWeatherForDate = (date: string) => {
    // Deterministic mock weather based on date string
    const day = new Date(date).getDate();
    const temp = 15 + (day % 10);
    const conditions = [
        { label: 'Sunny', icon: <CloudSun size={28} strokeWidth={2.5} className="text-pop-dark" />, bg: 'bg-pop-yellow' },
        { label: 'Cloudy', icon: <CloudSun size={28} strokeWidth={2.5} className="text-pop-dark" />, bg: 'bg-pop-blue' },
        { label: 'Partly Cloudy', icon: <CloudSun size={28} strokeWidth={2.5} className="text-pop-dark" />, bg: 'bg-pop-yellow' },
        { label: 'Clear', icon: <CloudSun size={28} strokeWidth={2.5} className="text-pop-dark" />, bg: 'bg-pop-yellow' }
    ];
...
const WeatherWidget: React.FC<{ date: string }> = ({ date }) => {
    const { temp, label, icon, bg } = getWeatherForDate(date);
    return (
        <div className="flex items-center gap-4 bg-white p-4 rounded-xl border-2 border-pop-dark shadow-pop mb-4 relative overflow-hidden group hover:shadow-pop-hover transition-all">
            <div className={`absolute top-0 right-0 w-24 h-full opacity-10 ${bg} skew-x-12 translate-x-12`}></div>
            <div className={`${bg} p-3 rounded-lg border-2 border-pop-dark text-pop-dark z-10`}>
                {icon}
            </div>
            <div className="z-10">
...
const ItineraryView: React.FC<ItineraryViewProps> = ({ activities, onToggleComplete, onUpdateActivity, onDeleteActivity, onPreviewImage, onImportCSV, onDownloadTemplate }) => {
    ...
    // Get unique sorted dates from activities or current selection
    const [extraDates, setExtraDates] = useState<string[]>([]);

    const dates = useMemo(() => {
        const activityDates = activities.map(a => a.date);
        const allDates = Array.from(new Set([...activityDates, ...extraDates])).sort();
        return allDates.length > 0 ? allDates : [new Date().toISOString().split('T')[0]];
    }, [activities, extraDates]);
    ...
    const handleAddDate = () => {
        ...
        setExtraDates(prev => [...prev, dateStr]);
            ...
            <div className="pt-8 pb-4 px-6 bg-white border-b-2 border-pop-dark shadow-sm z-10">
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2 snap-x">
                    {dates.map((date) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex flex-col items-center justify-center min-w-[70px] h-[80px] rounded-xl border-2 transition-all snap-start ${isSelected
                    ...
                    <button
                        ...
                        className="flex flex-col items-center justify-center min-w-[70px] h-[80px] rounded-xl border-2 border-dashed border-gray-300 text-gray-400 hover:border-pop-blue hover:bg-blue-50 transition-all snap-start"
                    >
                        <Plus size={20} strokeWidth={3} />
                        <span className="text-[10px] font-black uppercase mt-1">Day</span>
                    </button>
                </div>
            </div>
                        ...
                        return (
                            <div key={activity.id} className="relative pl-4">
                                ...
                                <div className="flex items-start gap-4 relative z-10">
                                    ...
                                    >
                                        <div className={`p-4 rounded-xl border-2 border-pop-dark transition-all group active:scale-[0.98] ${activity.isCompleted
                                            ? 'bg-gray-100 opacity-70'
                                            : isPast
                                                ? 'bg-white' // No shadow for past items
                                                : 'bg-white shadow-pop' // Shadow for future items
                                            }`}>
                                            <div className="flex justify-between items-start mb-2">
                                                <div className={`px-2 py-1 rounded border-2 border-pop-dark text-[10px] font-black uppercase flex items-center gap-1 ${activity.isCompleted ? 'bg-gray-200 text-gray-500' : 'bg-pop-yellow text-pop-dark'
                                                    }`}>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {Object.values(ActivityType).map(t => (
                                                <button
                                                    ...
                                                    className={`p-3 rounded-xl border-2 font-bold text-xs uppercase flex items-center gap-2 transition-all ${editForm.type === t
                                                        ? 'bg-pop-yellow border-pop-dark text-pop-dark shadow-pop-sm'
                                                        : 'bg-white border-gray-200 text-gray-400'
                                                        }`}
                                                >
                                                    {getActivityIcon(t as ActivityType)} {t}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
